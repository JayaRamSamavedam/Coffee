...
userSchema.methods.editUserDetails = async function(updates) {
  const allowedUpdates = ["fullName", "phonenumber", "birthday", "region", "gender","payementmode"];
...
// Create delivery address
userSchema.methods.createPreference = async function(preferences) {
  this.preferences = preferences;
  await this.save();
  return this.preferences;
};

userSchema.methods.editPreference = async function(updated) {
  if (!this.preferences) {
    this.preferences = updated;
  } else {
    Object.assign(this.preferences, updated);
  }
  await this.save();
  return this.preferences;
};
