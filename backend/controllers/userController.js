...
export const userRegister = async (req, res) => {
  ...
  try {
    ...
    if (preuser) {
      ...
      const userregister = new users({ fullName, email, phonenumber, password,userGroup:"customer" });
...
export const editUserData = async(req,res)=>{
  ...
  try
 { 
  const {fullName,phonenumber,payementmode,birthday,gender,region} = req.body;
 
  const u = await users.findOne({email:req.user.email})
  const saveduser = await u.editUserDetails({fullName,phonenumber,payementmode,birthday,gender,region});
  console.log("dj")
...
export const addPreference = async(req,res)=>{
  ...
  try{
    const u = await users.findOne({email:req.user.email})
    const saveduser = await u.createPreference(req.body.preference);
    return res.status(200).json({message:"deliveryAddress details added successfully"});
  }
  catch(error){
    console.log(error);
...
export const editPreference = async(req,res)=>{
  try{
    const u = await users.findOne({email:req.user.email});
    const saveduser = await u.editPreference(req.body.preference);
